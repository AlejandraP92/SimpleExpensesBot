...
def on_add(message):
	...
	parts = re.match(
		r"^sumar ([+-]?([0-9]*[.])?[0-9]+) y ([+-]?([0-9]*[.])?[0-9]+)$", message.text)
...
def on_add(message):
	...
	parts = re.match(
		r"^restar ([+-]?([0-9]*[.])?[0-9]+) y ([+-]?([0-9]*[.])?[0-9]+)$", message.text)
...
def on_add(message):
	...
	parts = re.match(
		r"^multiplicar ([+-]?([0-9]*[.])?[0-9]+) y ([+-]?([0-9]*[.])?[0-9]+)$", message.text)
...
def on_add(message):
	...
	parts = re.match(
		r"^dividir ([+-]?([0-9]*[.])?[0-9]+) y ([+-]?([0-9]*[.])?[0-9]+)$", message.text)
	...
	if oper2 == 0:
		bot.send_message(
		message.chat.id,
		"\U0001F63F Ups, no se puede divir por 0")
